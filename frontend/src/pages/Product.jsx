...
const Product = () => {
  ...
  return productData ? (
    <div className='px-4 sm:px-[5vw] md:px-[7vw] lg:px-[9vw]  border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100 '>
      {/*------------------ Product Data ---------------*/}
      <div className='flex gap-12 sm:gap-12 flex-row sm:flow-row'>
        {/* --------- Product Images --------------*/}
          ...
          <div className='flex flex-col gap-4 my-8'>
            ...
            <div className='flex gap-2'>
              {productData.sizes.map((item,index)=>(
                <button onClick={()=>setSize(item)} className={`border py-2 px-4 bg-gray-100 ${item === size ? 'border-orange-700 bg-orange-300' : ''}`} key={index}>{item}</button>
              ))}
            </div>
          </div>
      ...
      <div className='mt-20'>
        ...
        <div className='flex flex-col gap-4 border px-6 py-6 text-sm text-gray-500'>
          <p>An e-commerce website is an online platform that facilitates the buying and selling of products or services over the internet. 
            ...
            and conduct transactions without the need for a physical presence. E-commerce websites have gained immense popularity due 
            to their convenience, accessibility, and the global reach they offer.</p>
